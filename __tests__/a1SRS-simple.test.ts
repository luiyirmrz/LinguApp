...
/**
 * A1 SRS Simple Test Suite
 * Tests the A1 SRS configuration without React Native dependencies
 */

// Import the configuration directly
const A1_SRS_CONFIG_SIMPLE = {
...
const mockCroatianA1VocabularySimple = [
  {
    id: 'hr_a1_001',
    word: 'zdravo',
    translation: 'hello',
...
const mockCroatianA1GrammarSimple = [
  {
    id: 'hr_a1_gram_001',
...
describe('A1 SRS Configuration', () => {
  test('should have correct default parameters', () => {
    expect(A1_SRS_CONFIG_SIMPLE.DEFAULT_EASE_FACTOR).toBe(2.5);
    expect(A1_SRS_CONFIG_SIMPLE.INITIAL_INTERVAL).toBe(1);
    expect(A1_SRS_CONFIG_SIMPLE.INITIAL_REPETITIONS).toBe(0);
    expect(A1_SRS_CONFIG_SIMPLE.PASSING_QUALITY).toBe(3);
  });

  test('should have valid ease factor range', () => {
    expect(A1_SRS_CONFIG_SIMPLE.MIN_EASE_FACTOR).toBeLessThan(A1_SRS_CONFIG_SIMPLE.MAX_EASE_FACTOR);
    expect(A1_SRS_CONFIG_SIMPLE.MIN_EASE_FACTOR).toBe(1.3);
    expect(A1_SRS_CONFIG_SIMPLE.MAX_EASE_FACTOR).toBe(2.5);
  });

  test('should have ascending interval progression', () => {
    expect(A1_SRS_CONFIG_SIMPLE.INITIAL_INTERVAL).toBeLessThan(A1_SRS_CONFIG_SIMPLE.SECOND_INTERVAL);
    expect(A1_SRS_CONFIG_SIMPLE.SECOND_INTERVAL).toBeLessThan(A1_SRS_CONFIG_SIMPLE.THIRD_INTERVAL);
  });

  test('should have difficulty modifiers for all levels', () => {
    expect(A1_SRS_CONFIG_SIMPLE.DIFFICULTY_MODIFIERS.easy).toBe(1.2);
    expect(A1_SRS_CONFIG_SIMPLE.DIFFICULTY_MODIFIERS.medium).toBe(1.0);
    expect(A1_SRS_CONFIG_SIMPLE.DIFFICULTY_MODIFIERS.hard).toBe(0.8);
    expect(A1_SRS_CONFIG_SIMPLE.DIFFICULTY_MODIFIERS.very_hard).toBe(0.6);
  });
});
...
describe('A1 SRS Performance Calculation', () => {
  test('should calculate correct interval for first review', () => {
    ...
    const secondInterval = A1_SRS_CONFIG_SIMPLE.SECOND_INTERVAL;
  ...
  test('should reset interval for failed reviews', () => {
    ...
    const expectedInterval = A1_SRS_CONFIG_SIMPLE.INITIAL_INTERVAL;
    expect(expectedInterval).toBe(1);
  });

  test('should apply difficulty modifiers correctly', () => {
    const easyModifier = A1_SRS_CONFIG_SIMPLE.DIFFICULTY_MODIFIERS.easy;
    const hardModifier = A1_SRS_CONFIG_SIMPLE.DIFFICULTY_MODIFIERS.hard;
...
describe('A1 Croatian Vocabulary Integration', () => {
  test('should have Croatian A1 vocabulary loaded', () => {
    expect(mockCroatianA1VocabularySimple).toBeDefined();
    expect(mockCroatianA1VocabularySimple.length).toBeGreaterThan(0);
  });

  test('should have Croatian A1 grammar loaded', () => {
    expect(mockCroatianA1GrammarSimple).toBeDefined();
    expect(mockCroatianA1GrammarSimple.length).toBeGreaterThan(0);
  });

  test('should have vocabulary items with correct structure', () => {
    const firstVocab = mockCroatianA1VocabularySimple[0];
  ...
  test('should have grammar concepts with correct structure', () => {
    const firstGrammar = mockCroatianA1GrammarSimple[0];
  ...
  test('should have appropriate difficulty levels for A1', () => {
    const vocabDifficulties = mockCroatianA1VocabularySimple.map(v => v.difficulty);
    const grammarDifficulties = mockCroatianA1GrammarSimple.map(g => g.difficulty);
...
describe('A1 SRS Performance Modifiers', () => {
  ...
  test('should calculate performance modifier for hints used', () => {
    ...
    const modifier = calculateA1PerformanceModifier(performance);
    expect(modifier).toBeCloseTo(0.99, 2); // 1.0 * 1.1 * 0.9 = 0.99 (fast response + penalty)
  });
...
describe('A1 SRS Integration', () => {
  test('should create SRS items for A1 lesson', () => {
    ...
    const mockVocabulary = mockCroatianA1VocabularySimple.slice(0, 3);
    const mockGrammar = mockCroatianA1GrammarSimple.slice(0, 2);
  ...
  test('should validate SRS configuration parameters', () => {
    ...
    expect(A1_SRS_CONFIG_SIMPLE.MIN_EASE_FACTOR).toBeLessThan(A1_SRS_CONFIG_SIMPLE.MAX_EASE_FACTOR);
    expect(A1_SRS_CONFIG_SIMPLE.DEFAULT_EASE_FACTOR).toBeGreaterThanOrEqual(A1_SRS_CONFIG_SIMPLE.MIN_EASE_FACTOR);
    expect(A1_SRS_CONFIG_SIMPLE.DEFAULT_EASE_FACTOR).toBeLessThanOrEqual(A1_SRS_CONFIG_SIMPLE.MAX_EASE_FACTOR);
    
    // Test interval progression
    expect(A1_SRS_CONFIG_SIMPLE.INITIAL_INTERVAL).toBeGreaterThan(0);
    expect(A1_SRS_CONFIG_SIMPLE.SECOND_INTERVAL).toBeGreaterThan(A1_SRS_CONFIG_SIMPLE.INITIAL_INTERVAL);
    expect(A1_SRS_CONFIG_SIMPLE.THIRD_INTERVAL).toBeGreaterThan(A1_SRS_CONFIG_SIMPLE.SECOND_INTERVAL);
    
    // Test quality thresholds
    expect(A1_SRS_CONFIG_SIMPLE.PASSING_QUALITY).toBeGreaterThanOrEqual(0);
    expect(A1_SRS_CONFIG_SIMPLE.PASSING_QUALITY).toBeLessThanOrEqual(5);
    expect(A1_SRS_CONFIG_SIMPLE.EXCELLENT_QUALITY).toBeGreaterThanOrEqual(A1_SRS_CONFIG_SIMPLE.PASSING_QUALITY);
  });
});
